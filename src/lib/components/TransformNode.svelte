...
<Box class="transform-node" surface={2} border radius={3} shadow={2} {tui}>
  <Box class="node-header transform-header" surface={3} {tui}>
    <span class="node-icon">🔄</span>
    <Text class="node-title">{node.label || 'Transform'}</Text>
    <span class="node-badge">transform</span>
  </Box>
...
<style>
  ...
  :global(.transform-node .node-header) {
    ...
    background: linear-gradient(135deg, rgba(255, 152, 0, 0.15) 0%, transparent 100%) !important;
  }

  .node-icon {
    font-size: 18px;
    color: var(--node-accent-transform);
  }
