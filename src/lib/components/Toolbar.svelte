...
<StatusBar {tui} width={collapsed ? '52px' : '200px'}>
  <div class="toolbar-inner" class:collapsed>
    ...
    <div class="toolbar-section toolbar-section--meta">
      {#if !collapsed}<h3>App</h3>{/if}
      <Button {tui} onclick={() => onOpenSettings?.()} class="toolbar-btn" title="Settings">
        <span class="btn-icon" aria-hidden="true">⚙️</span><span class="btn-text"> Settings</span>
      </Button>
      <Button {tui} onclick={() => onOpenHelp?.('shortcuts')} class="toolbar-btn" title="Keyboard shortcuts">
        <span class="btn-icon" aria-hidden="true">⌨️</span><span class="btn-text"> Shortcuts</span>
      </Button>
      <Button {tui} onclick={() => onOpenHelp?.('about')} class="toolbar-btn" title="About RuneBook">
        <span class="btn-icon" aria-hidden="true">ℹ️</span><span class="btn-text"> About</span>
      </Button>
    </div>
  </div>
</StatusBar>
