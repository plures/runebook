...
describe('component-registry module', () => {
  describe('registerComponent', () => {
    it('adds a component to the registry', () => {
      ...
      expect(engine.getContext().components['terminal'].lifecycle).toBe('registered');
    });
  });
  ...
  describe('unregisterComponent', () => {
    it('marks a component as removed', () => {
      ...
      const result = engine.step([UnregisterComponentEvent.create({ type: 'terminal' })]);
      const unregFact = result.state.facts.find(f => f.tag === COMPONENT_UNREGISTERED_FACT);
      expect(unregFact).toBeDefined();
      expect(engine.getContext().components['terminal'].lifecycle).toBe('removed');
    });
  });
  ...
  describe('checkPortCompatibility', () => {
    ...
    it('emits PORT_COMPATIBLE for matching types', () => {
      ...
      const compat = result.state.facts.find(f => f.tag === PORT_COMPATIBLE_FACT);
      expect(compat).toBeDefined();
      expect(engine.getContext().portCompatibilityResult?.compatible).toBe(true);
    });
    ...
    it('emits PORT_INCOMPATIBLE for mismatched types', () => {
      ...
      const incompat = result.state.facts.find(f => f.tag === PORT_INCOMPATIBLE_FACT);
