...
const memoryBudgetRule = defineRule<ResourceManagementContext>({
  ...
  description:
    "Detect memory pressure when usage exceeds the configured budget",
...
const memoryBudgetConstraint = defineConstraint<ResourceManagementContext>({
  ...
  description:
    "Memory usage must remain within budget when a budget is configured",
  impl: (state) => {
...
export const resourceManagementModule: PraxisModule<ResourceManagementContext> =
  defineModule({
    rules: [
      processLimitRule,
      releaseTerminalRule,
